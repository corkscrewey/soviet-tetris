...
ARG REPO="corkscrewey/soviet-tetris"
...
RUN curl -LO https://github.com/${REPO}/releases/download/files/disks.zip \
	&& unzip disks.zip \
	&& curl -LO https://raw.githubusercontent.com/${REPO}/master/files/config/pdp.ini
...
WORKDIR /sim
COPY --from=build /build/*.dsk /build/pdp.ini /sim/
